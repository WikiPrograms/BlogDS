			...
			if(actual->dato==nodobuscar){ //si el valor del actual es igual al nodo buscado, lo encontramos, y si lo encontramos podemos eliminarlo
				...
				if(actual == inicio){
					...
					ultimo=anterior; //ahora toma posicion del ultimo, inicio-> anterior->NULL
						...
						//porque el nodo al que su apuntador a siguiente apunta, el nodo apuntado tiene un apuntador atras, entonces el apuntador siguiente del nodo a eliminar
						//le dice que el nodo siguiente del eliminado, se ligue con el anterior del eliminado, asi se hace la doble ligadura.
					}
				}
